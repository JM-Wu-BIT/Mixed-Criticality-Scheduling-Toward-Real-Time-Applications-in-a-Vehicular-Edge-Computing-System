function itsStartTime = PSO_startTimeCalFuc(taskID, vmID, vmCELLs, itsPreTasks, DAG_comCost)
    ...
    for i = 1:size(vmCELLs, 2)
        ...
        if ~isempty(taskRunTimeInfo)
            for j = 1:size(taskRunTimeInfo, 1)
                preTaskID = taskRunTimeInfo(j, 5);
                if ismember(preTaskID, itsPreTasks)
                    if isequal(i, vmID)
                        itsStartTimeTemp = taskRunTimeInfo(j, end-4)+taskRunTimeInfo(j, end-3);
                    else
                        itsStartTimeTemp =taskRunTimeInfo(j, end-4)+ taskRunTimeInfo(j, end-3) + DAG_comCost(j, taskID);
                    end
